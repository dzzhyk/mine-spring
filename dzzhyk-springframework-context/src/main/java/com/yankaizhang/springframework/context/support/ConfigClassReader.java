...
public class ConfigClassReader {
     ...
     * 解析配置类，注册配置类中的Bean对象
     */
    public Set<RootBeanDefinition> parseAnnotationConfigClass(Class<?> clazz) {
        Set<RootBeanDefinition> definitions = new HashSet<>(doParseAnnotationConfigClass(clazz));
        // 获取内部类
        Class<?>[] declaredClasses = clazz.getDeclaredClasses();
        for (Class<?> declaredClass : declaredClasses) {
            List<RootBeanDefinition> beanDefinitions = doParseAnnotationConfigClass(declaredClass);
    ...
    private List<RootBeanDefinition> doParseAnnotationConfigClass(Class<?> configClass) {
        Method[] declaredMethods = configClass.getDeclaredMethods();
        ArrayList<RootBeanDefinition> definitions  = new ArrayList<>(declaredMethods.length);
     ...
     * 解析BeanMethod
     */
    private RootBeanDefinition parseBeanMethod(Class<?> configClass, Method method){
        ...
        RootBeanDefinition definition = reader.doCreateConfigClassBeanDefinition(factoryBeanName, factoryBeanClassName,
